...
def build_feature_extractor(cfg, adv=False, with_permute_adain=True):
    model_name, backbone_name = cfg.MODEL.NAME.split('_')
    if backbone_name.startswith('resnet'):
        return_layers = {'layer4': 'out'}
        backbone = resnet_feature_extractor(backbone_name, pretrained_weights=cfg.MODEL.WEIGHTS, aux=False, pretrained_backbone=True, freeze_bn=cfg.MODEL.FREEZE_BN, return_layers=return_layers, adv=adv, with_permute_adain=with_permute_adain)
...
def build_classifier(cfg):
    base_name, backbone_name = cfg.MODEL.NAME.split('_')
